...
This is achieved by dividing the $$ x $$ and $$ y $$ directions into $$ N_x $$ and $$ N_y $$ elements, where for a square domain we have $$ N = N_x = N_y $$. In general, if $$ L_x $$ and $$ L_y $$ represent the dimensions in the $$ x $$ and $$ y $$ directions, we have
...
for $$ i = 0, 1, \dots, N_x $$ and $$ j = 0, 1, \dots, N_y $$. The boundary conditions are specified on the edges of the domain. For example, if the head values are known to be $$ \bar{h}_1 $$ along $$ x = 0 $$ and $$ x = L_x $$ and $$ \bar{h}_2 $$ along $$ y = 0 $$ and $$ y = L_y $$, we write
...
Using these in the simplified governing equation gives
...
This equation is represented by the five-point stencil with the highlighted nodes in the previous figure. It implies that the hydraulic head at point $$ (x_i, y_j) $$ is obtained by averaging the values of neighboring points. The system of equations can now be derived for arbitrarily chosen grid meshes.

### Solution for $N_x = N_y = 4$

Consider a $$ 1~\mathrm{m} \times 1~\mathrm{m} $$ grid divided into an equal number of elements $$ N_x = N_y = 4 $$ in both directions ($$ N_x + 1 = N_y + 1 = 5 $$ nodes in each direction) as shown in the figure below. The grid involves 25 nodes which are numbered as shown.

![2D 4x4 grid](assets/figs/2D_4x4_grid.png){: .center-image }
...
A two-dimensional color contour plot of this solution is shown in the figure below with 10 contour levels and linear interpolation between neighboring nodes.

![Solution for a 4x4 grid](assets/figs/Solution_for_4x4_grid.png){: .center-image }
*Solution for $$N_x = N_y = 4$$*

### Solution for an $N_x \times N_y$ Grid

In general, we want to obtain a solution on an arbitrarily discretized grid. Let the domain be discretized into $$ N_x \times N_y $$ elements ($$ (N_x + 1) \times (N_y + 1) $$ nodes) where $$ N_x $$ and $$ N_y $$ are chosen such that $$ \Delta x = \Delta y $$, i.e. a uniform grid. The coefficient matrix $$ \mathbf A $$ will have a similar block structure as described in the previous section but with more element matrices $$ \mathbf D $$ and $$ \mathbf I $$. The size of matrix $$ \mathbf A $$ will be $$ (N_x - 1)^2 \times (N_y - 1)^2 $$ and its block form may be written as

$$
\mathbf A = \left[ \begin{array}{c|c|c|c|c}
\mathbf D & \mathbf I & \mathbf O & \cdots & \mathbf O \\
\hline
\mathbf I & \mathbf D & \mathbf I & \ddots & \vdots \\
\hline
\mathbf O & \mathbf I & \ddots & \ddots & \mathbf O \\
\hline
\vdots & \ddots & \ddots & \mathbf D & \mathbf I \\
\hline
\mathbf O & \cdots & \mathbf O & \mathbf I & \mathbf D \\
\end{array} \right].
$$

The sizes of the matrices $$ \mathbf D $$ and $$ \mathbf I $$ change depending on the selected $$ N_x $$ and $$ N_y $$ values. For the particular example considered here where the boundary conditions are known along the four boundaries, the matrices $$ \mathbf D $$ and $$ \mathbf I $$ will have sizes of $$ (N_x - 1) \times (N_y - 1) $$ and may be written as
